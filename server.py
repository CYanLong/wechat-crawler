...
def pro_message():
	...
	guide = """以下是本订阅号的使用指南:\n
	1:回复 检索 书名, 例如: 检索 黑客与画家 得到黑大图书馆藏信息\n
	2:回复 段子或笑话 可得到一条笑话\n
	3:回复 电影推荐 可得到一部电影
	也可在后面加一个电影类型 例如: 电影推荐 爱情 
	类型可以是豆瓣电影支持的任何类型,
	例如:爱情, 喜剧, 经典, 情色, 同志 等,详见:https://movie.douban.com/tag/  可得到一部关于此分类的电影\n
	4:回复 电影 电影名,例如: 电影 美国往事  可得到关于此电影的详细信息.(来源于豆瓣)"""
	...
	if size == 1:
		key = li_mess[0]
		if "段子" in key or "笑话" in key:
			uid = req_xml.find('FromUserName').text
			content = joke(uid)
			return resp(respMessage(req_xml, content))
		elif "电影推荐" in key:
			uid = req_xml.find('FromUserName').text
			title, desc, picUrl = movieByTag(uid)
